...
public class MdServiceImpl implements MdService {
    ...
    private RuntimeException wrapToQbmException(Exception ex) {
        if (ex instanceof ApiException) {
            ...
            throw new RuntimeException(
                    "访问MDM api失败, status code=[" + apiException.getCode() + "]:" + ex.getMessage(), ex);
        }
        throw new RuntimeException("访问MDM api失败, " + ex.getMessage());
    }
    ...
    @Override
    public Map<Integer, String> getTopLevelKnowledgePoints(int courseId) {
        List<KnowledgePoint> topLevelPoints = this.getKnowledgePointsByCourseId(courseId, 1);
        Map<Integer, String> map = new HashMap<>();
        for (KnowledgePoint point : topLevelPoints) {
            map.put(point.getId(), point.getName());
        }
        return map;
    }
    ...
    @Override
    public List<KnowledgePoint> getKnowledgePointsByCourseId(Integer courseId, Integer depth) {
        return null;
    }

    @Override
    public List<CatalogKPointMap> getCatalogKPointMappings(Integer courseId) {
        return null;
    }
}
